...
import Sinfelswiper from "@/components/swiperMayin";

export default function Page() {
  const [singleMovie, setSingleMovie] = useState({});
  const [props, setprops] = useState([]);

  const tite = usePathname();
  let id = tite.split("/")[tite.split("/").length - 1];
  useEffect(() => {
    async function fetchMovie() {
      const response = await fetch(
        `https://api.themoviedb.org/3/movie/${id}?api_key=95ecf50d83cc6a4046e34273fb02582d`
      );
